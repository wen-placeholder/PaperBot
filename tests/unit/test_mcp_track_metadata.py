...
class _FakeResearchStore:
    ...
    def get_track(self, *, user_id: str, track_id: int):
        ...
        if user_id == TEST_USER_ID and track_id == 42:
            return {
                "id": 42,
                "name": "ML",
        ...
        if user_id == TEST_USER_ID and track_id == 43:
...
class TestTrackMetadataResource:
    ...
    async def test_returns_track_metadata_for_valid_id(self):
        ...
        try:
            result = await mod._track_metadata_impl(user_id=TEST_USER_ID, track_id="42")
        finally:
            mod._store = None

        data = json.loads(result)
        assert fake_store.calls == [{"user_id": TEST_USER_ID, "track_id": 42}]
        assert data["id"] == 42
        assert data["name"] == "ML"
    ...
    async def test_returns_error_when_track_not_found(self):
        ...
        try:
            result = await mod._track_metadata_impl(user_id=TEST_USER_ID, track_id="99")
        finally:
            mod._store = None
    ...
    async def test_returns_error_for_non_integer_track_id(self):
        ...
        mod._store = _FakeResearchStore()
        try:
            result = await mod._track_metadata_impl(user_id=TEST_USER_ID, track_id="abc")
        finally:
            mod._store = None
    ...
    async def test_returns_error_when_track_is_archived(self):
        ...
        mod._store = fake_store
        try:
            result = await mod._track_metadata_impl(user_id=TEST_USER_ID, track_id="43")
        finally:
            mod._store = None
